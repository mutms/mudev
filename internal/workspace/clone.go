...
type Options struct {
	// Config is the resolved configuration (directories, git URL policy).
...
// assembleCore puts Moodle core in the workspace root, verifies it, and
// records it.
//
...
// verifyCore checks that the workspace root really holds the Moodle the recipe
