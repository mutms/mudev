...
func newRootCmd(version string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// --version identifies the binary: the stamped version plus the
		// release flavours compiled in, so "unknown flavour" warnings can be
		// checked against the build that printed them.
		Version: version + " (release flavours: " + strings.Join(workspace.KnownFlavours(), ", ") + ")",
...
// stamped into cmd/mudev at build time and passed through for --version.
