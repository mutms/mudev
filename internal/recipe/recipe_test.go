...
func TestFetchOrderMustBeAList(t *testing.T) {
	// `fetch_order: forge, origin` is one YAML string, not a list. mudev used
	// to ignore it silently and fetch in its default order, which looks exactly
	// like the setting not working; the schema now rejects it outright.
	_, err := Parse([]byte(`
extra:
  mudev: {release: mutms, fetch_order: forge, origin}
moodle:
