...
The full command reference. For what mudev is and the two things most people need — cloning a
recipe and adding a plugin — see the [README](../README.md); this document covers the whole
workspace workflow.
...
With the standard `mpd` layout, mudev finds `/srv/extra/mdl-plugins` and
`/srv/extra/mdl-recipes` automatically.
...
```sh
ssh mpd-NNN -A
mkdir -p /srv/projects/mutms52 && cd /srv/projects/mutms52   # mudev never creates the directory,
