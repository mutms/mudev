...
// to run a subprocess (git, moodle, workspace, …) builds a domain wrapper on
// top of this one — mirroring mpd's single-choke-point rule (VM/Exec.swift).
...
type Cmd struct {
	Name  string    // executable name or path; resolved via PATH when not absolute
